...
suite('EmployeeList', () => {
  ...
  test('has default properties', async () => {
    ...
    assert.isArray(element.filteredEmployees);
  ...
  test('loads mock data on initialization', async () => {
    const element = await fixture(html`<employee-list></employee-list>`);
    assert.isTrue(element.employees.length > 0);
    assert.property(element.employees[0], 'firstName');
    assert.property(element.employees[0], 'lastName');
    assert.property(element.employees[0], 'email');
    assert.property(element.employees[0], 'department');
    assert.property(element.employees[0], 'position');
  });

  test('initializes filteredEmployees with all employees', async () => {
    const element = await fixture(html`<employee-list></employee-list>`);
    assert.equal(element.filteredEmployees.length, element.employees.length);
  });
  ...
  test('calculates total pages correctly', async () => {
    ...
    assert.equal(element.totalPages, expectedPages);
  });
  ...
  suite('_getMockData', () => {
    test('returns array of employee objects', async () => {
      const element = await fixture(html`<employee-list></employee-list>`);
      const mockData = element._getMockData();
      assert.isArray(mockData);
      assert.isTrue(mockData.length > 0);
    });

    test('returns employees with required properties', async () => {
      const element = await fixture(html`<employee-list></employee-list>`);
      const mockData = element._getMockData();
      const requiredProps = [
        'id',
